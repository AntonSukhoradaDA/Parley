# Online Chat Server — Project Instructions
...
- A user becomes **offline** only when all browser tabs are closed / offloaded by the browser as inactive.
...
## 6. Advanced Requirements (Optional)

If you manage to implement the requirements above quickly, add the following additional feature: **Jabber (XMPP) protocol support**.

This should include:

1. Users must be able to connect to the server using a Jabber client (choose the Jabber protocol support level you can afford).
2. Servers must be able to support **federation** — i.e. messages between servers (this will require a more complicated setup in Docker Compose).
3. Use a Jabber library available for your tech stack.

**Most advanced possible implementation must include a load test for federation of two servers:**

1. 50+ clients connected to one server — A, 50+ to another — B.
2. Messaging from A to B and back.

> **Note:** With an existing library this feature is rather integration than coding, but with an AI agent it could be challenging.

Additionally, you must add Jabber-specific screens to the server UI:
...
- Create a **public GitHub repository** with your project.
- The project **MUST be buildable and runnable** by running:
